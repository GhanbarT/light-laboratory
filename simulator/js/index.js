...
window.onload = function (e) {
    ...
    document.getElementById('rayDensityPlus').onclick = function () {
        let rayDensityValue = Math.log(scene.rayDensity) + 0.1;
    ...
    document.getElementById('rayDensityMinus').onclick = function () {
        let rayDensityValue = Math.log(scene.rayDensity) * 1.0 - 0.1;
        scene.rayDensity = Math.exp(rayDensityValue);
        document.getElementById('rayDensity').value = rayDensityValue;
        /*document.getElementById('rayDensity_more').value = rayDensityValue;
        document.getElementById('rayDensity_mobile').value = rayDensityValue;*/
...
function loadLevel() {
    ...
    let level = pageName.slice(0, pageName.indexOf('.html'))
    ...
    fetch('./levels/' + level + '.json')
        ...
        .then((data) => {
            ...
            // Pass the created file to your openFile function
            openFile(file);
        })
        .catch((error) => {
...
function exportSVG(cropBox) {
    ...
    dpr = 1;
    let imageWidth = cropBox.p4.x - cropBox.p1.x;
    let imageHeight = cropBox.p4.y - cropBox.p1.y;
...
function exportImage(cropBox) {
    ...
    scene.origin = {x: -cropBox.p1.x * scene.scale, y: -cropBox.p1.y * scene.scale};
    dpr = 1;
    let imageWidth = cropBox.width;
    let imageHeight = cropBox.width * (cropBox.p4.y - cropBox.p1.y) / (cropBox.p4.x - cropBox.p1.x);
